...
lollipop = function(maf, gene = NULL, gff=gff, AACol = NULL, labelPos = NULL, labPosSize = 0.9, showMutationRate = TRUE,
                        showDomainLabel = TRUE, refSeqID = NULL, proteinID = NULL, cBioPortal = FALSE,
  ...
  geneID = gene
  mut = maf[grepl("^p|P",maf$AA_CHANGE), ]
  ...
  prot.dat = data.table::data.table(mut[,c("VAR_TYPE_SX","AA_CHANGE")])
  ...
  names(col) = names = c('Missense_Mutation','Gene Amplification','Gene Homozygous Deletion','Substitution/Indel',
                         'IGR','Fusion/Rearrangement','Splice_Site','Intron','Frame_Shift_Ins',
                         'In_Frame_Dell','In_Frame_Del','ITD','In_Frame_Ins','Translation_Start_Site',
                         'Multi_Hit','Truncation')
  
  #prot.dat = prot.dat[ ,Variant_Classification != 'Splice_Site']
  #Remove 'p.'
  prot.spl = strsplit(x = as.character(prot.dat$AA_CHANGE), split = '.', fixed = TRUE)
  ...
  prot.snp.sumamry = prot.dat[,.N, .(VAR_TYPE_SX, conv, pos)]
  ...
  col = col[unique(as.character(prot.snp.sumamry[,VAR_TYPE_SX]))]
  ...
  title(main = geneID, adj = 0, font = 2, cex.main = titleSize[1], line = 0.8)
  ...
  if(printCount){
    print(prot.snp.sumamry[,.(pos, conv, count, VAR_TYPE_SX)][order(pos)])
